...
// VibeSync mod: skip Spotify OAuth entirely. Our backend is open (no auth).
// Always returns a mock token and dispatches fetchUser → /sp/v1/me on Render.
export const loginToSpotify = createAsyncThunk<{ token?: string; loaded: boolean }>(
  'auth/loginToSpotify',
  async (_, thunkAPI) => {
    thunkAPI.dispatch(fetchUser());
    return { token: 'vibesync-mock-token', loaded: true };
  }
);
